...
SensorNode::SensorNode() :
	...
	status = CONFIG_SOLICIT;
	controllers = new ConfigurationController*[5];
	controllersSize = 0;
}
		...
		case CONFIG_SOLICIT: {
			...
			if (header->MessageType == CONFIG_ADVERTISE_TYPE) {
				if (this->ApplyConfig(
					...
					if (status == CONFIG_ACCEPT) {
						// We need to send a config accept
						collectorNodeId = header->SourceNode;
						status = OPERATIONAL;
					}
				} else {
...
void SensorNode::SolicitConfig() {
	...
	// Send
	rf12.Send((byte) 0, message, (uint8_t) CONFIG_SOLICIT_LENGTH, 0 /*false*/);
}
...
bool SensorNode::ApplyConfig(byte* message, byte len) {
	...
	while (1) {
		...
		if (message[pos] == 0 && message[pos + 1] == 0) {
			...
			status = CONFIG_ACCEPT;
			return true;
		}
		for (byte controllerIndex = 0; controllerIndex < controllersSize; controllerIndex++) {
			if (message[pos] == controllers[controllerIndex]->getType()) {
				Serial.print("Found controller, handling message with size ");
				Serial.println((int) message[pos + 1]);
				if (!(controllers[controllerIndex]->Handle(message + pos + 2,
						message[pos + 1])))
					return false;
				handled = true;
				break;
...
void SensorNode::Register(ConfigurationController* controller) {
	...
	Serial.print((int) controllersSize);
	Serial.println(".");
	if (controllersSize < MAX_CONTROLLERS) {
		controllers[controllersSize++] = controller;
	}

}

void SensorNode::SendConfigAccept(byte* message, byte len) {
	byte dest = collectorNodeId;
